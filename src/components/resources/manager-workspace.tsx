...
export function ResourceManager({ mode = "local", email }: { mode?: ManagerMode; email?: string }) {
  ...
  const [collectionErrorStatus, setCollectionErrorStatus] = useState(0);
  const collectionRequestPending = useRef(false);
  ...
  const collectionActionStale = collectionAction !== null && (collectionAction.context.mode !== mode || mode === "cloud" && collectionAction.context.libraryRevision !== libraryRevision);
  ...
  function openCollectionAction(resource: LibraryResource) {
    if (pending || loading || collectionRequestPending.current) return;
    setCollectionError(""); setCollectionErrorStatus(0);
    setCollectionAction({ resource: structuredClone(resource), context: { ...context }, restore: resource.status === "archived", published: publishedIdSet.has(resource.id) });
  }

  function closeCollectionAction() {
    if (!collectionRequestPending.current) setCollectionAction(null);
  }

  async function confirmCollectionAction() {
    const target = collectionAction;
    if (!target || pending || loading || collectionRequestPending.current || collectionActionStale || [404, 409, 428].includes(collectionErrorStatus)) return;
    collectionRequestPending.current = true;
    setPending("collection-action"); setCollectionError(""); setCollectionErrorStatus(0); setMessage("");
    try {
      const result = await managerRequest<ManagerLibraryResult>({ action: "bulk", ids: [target.resource.id], changes: { status: target.restore ? "organized" : "archived", visibility: "private" } }, target.context);
      updateResources(result, `“${target.resource.name}”${target.restore ? "已恢复为已整理，仅自己可见。" : "已移到“已移出”，可随时恢复收藏。"}${target.published ? "网站仍在展示，需另行发布更新。" : ""}`);
      setSelected(previous => { const next = new Set(previous); next.delete(target.resource.id); return next; });
      setCollectionAction(null);
      window.requestAnimationFrame(() => searchRef.current?.focus({ preventScroll: true }));
    } catch (failure) { setCollectionError(managerError(failure)); setCollectionErrorStatus(managerErrorStatus(failure)); }
    finally { collectionRequestPending.current = false; setPending(""); }
  }
  ...
  async function bulk(changes: Record<string, unknown>) {
    ...
    try {
      ...
      updateResources(result, `已更新 ${selected.size} 条收藏。${mode === "cloud" ? "修改已保存到私人云端，发布到网站后公开页才会更新。" : changes.status === "archived" ? "已移出内容不再作为公开候选，重新生成公开版本后生效。" : "修改已保存在本机。"}`); setSelected(new Set());
  ...
  const sidebarContent = <>
    ...
    <div className={styles.navDivider} /><nav aria-label="维护工具" className={styles.navGroup}>{mode === "cloud" ? <><Link className={smartStyles.managerNavLink} href="/tools/manage/imports"><ManagerIcon name="clock" /><span>导入记录</span></Link><Link className={smartStyles.managerNavLink} href="/tools/manage/settings"><ManagerIcon name="grid" /><span>AI 连接设置</span></Link></> : <button type="button" onClick={() => setDialog("history")}><ManagerIcon name="clock" /><span>导入记录</span></button>}<button type="button" className={styles.collectionTextButton} aria-pressed={view === "archived"} onClick={() => changeView("archived")}><ManagerIcon name="archive" /><span>已移出</span><small>{resources.length - activeResources.length}</small></button><button type="button" disabled={Boolean(pending) || loading} onClick={backup}><ManagerIcon name="download" /><span>{pending === "backup" ? "正在下载…" : "下载完整备份"}</span></button></nav>
  ...
  return <section className={styles.workspace}><Suspense fallback={null}><ManagerLocationObserver /></Suspense>
    ...
    <div className={styles.workspaceGrid}><aside className={styles.sidebar}>{sidebarContent}</aside><div className={styles.main}>
      ...
      {(batchFilterId || resourceFilterId) ? <div className={smartStyles.batchFilter}><span>{batchFilterId ? "正在查看本批新增的收藏" : "正在查看指定收藏"}</span><div>{batchFilterId ? <Link className={smartStyles.textLink} href={`/tools/manage/imports/${encodeURIComponent(batchFilterId)}`}>返回整理结果</Link> : null}<button type="button" className={smartStyles.textLink} onClick={clearBatchFilter}>查看全部收藏</button></div></div> : null}{pins.length && view === "all" && !category && !query && !kind && !batchFilterId && !resourceFilterId ? <section className={styles.pins} aria-label="我的常用入口"><p>常用入口 <span>随手置顶，随时回来</span></p><div>{pins.map(item => <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer"><ManagerResourceIcon name={item.name} icon={item.icon} /><strong>{item.name}</strong><ManagerIcon name="arrow" /></a>)}</div></section> : null}
    ...
    {collectionAction ? <ManagerDialog title={collectionAction.restore ? "恢复这条收藏？" : "移出这条收藏？"} onClose={closeCollectionAction} busy={Boolean(pending)}>
      <div className={styles.collectionActionBody}>
        <p className={styles.collectionActionName}>{collectionAction.resource.name}</p>
        <p>{collectionAction.restore ? "恢复为已整理，仅自己可见。" : "操作后会移到“已移出”，可在那里随时恢复收藏。"}</p>
        {collectionAction.published ? <p className={styles.collectionPublicationNote}>网站仍在展示，需另行发布更新。</p> : null}
        {collectionActionStale || collectionError ? <ManagerErrorNotice message={collectionActionStale ? "收藏库已更新。请刷新列表，重新确认这条资源。" : collectionError} status={collectionActionStale ? 409 : collectionErrorStatus} onRefresh={() => { if (!collectionRequestPending.current) { setCollectionAction(null); refreshList(); } }} /> : null}
        <div className={styles.dialogActions}>
          <button type="button" className={styles.secondaryButton} disabled={Boolean(pending)} onClick={closeCollectionAction}>取消</button>
          <button type="button" className={styles.primaryButton} disabled={Boolean(pending) || loading || collectionActionStale || [404, 409, 428].includes(collectionErrorStatus)} onClick={() => void confirmCollectionAction()}>{pending === "collection-action" ? "正在保存…" : collectionAction.restore ? "恢复收藏" : "移出收藏"}</button>
        </div>
      </div>
    </ManagerDialog> : null}
