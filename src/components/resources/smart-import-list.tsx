...
export function SmartImportList() {
  ...
  return <>
    <header className={styles.pageHeader}><div><h1>导入记录</h1><p>导入后先看分类结果，改好就能加入收藏。</p></div><button type="button" className={styles.primaryButton} onClick={() => setUploadOpen(true)}><ManagerIcon name="upload" />导入书签</button></header>
    <div className={styles.importIntro}><span><ManagerIcon name="check" />自动整理重复链接</span><span><ManagerIcon name="folder" />直接查看拟用分类</span><span><ManagerIcon name="collection" />由你确认加入收藏</span></div>
