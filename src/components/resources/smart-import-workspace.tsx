...
export function SmartImportWorkspace({ batchId }: { batchId: string }) {
  ...
  const [loadedAIView, setLoadedAIView] = useState(false);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setData(result);
        setLoadedAIView(filters.suggestionSource === "model");
  ...
  function changeView(next: ResultView, nextSource: SourceView = null) { setView(next); setSourceView(nextSource); setShowCollected(false); setShowAI(false); setPage(1); setSearch(""); setFilters(nextSource ? { view: nextSource } : { resultStatus: next }); setLoading(true); }
  function changeFilter(next: Partial<SmartImportFilters>) { setFilters(previous => ({ ...previous, ...next })); setPage(1); setLoading(true); }
  function clearFilters() { setSearch(""); setPage(1); setFilters(showAI ? { suggestionSource: "model" } : sourceView ? { view: sourceView } : { resultStatus: showCollected ? "collected" : view }); setLoading(true); }
  ...
  function showCollectedResources() { setShowCollected(true); setShowAI(false); setSourceView(null); setFilters({ resultStatus: "collected" }); setPage(1); setSearch(""); setLoading(true); }
  function showAIResults() { setShowAI(true); setShowCollected(false); setSourceView(null); setFilters({ suggestionSource: "model" }); setPage(1); setSearch(""); setLoading(true); resultsPanel.current?.scrollIntoView({ block: "start" }); aiHeading.current?.focus({ preventScroll: true }); }
  ...
  // Keep a stale page from being labelled as AI results while switching views.
  const listData = showAI === loadedAIView ? data : null;
  ...
  function resourceRow(group: SmartImportGroup) {
    ...
    return <li key={group.id} className={`${styles.resultRow} ${selected.has(group.id) ? styles.resultRowSelected : ""}`}>
      ...
      <div className={styles.resultResource}><div className={styles.resultResourceHeading}><button type="button" className={styles.resultName} onClick={() => setDetail(group.id)}>{proposal.name || "未命名资源"}</button><span className={styles.resultDomain}>{domainOf(group.representative.url)}</span></div>{proposal.description ? <p className={styles.resultDescription}>{proposal.description}</p> : null}<div className={styles.resultMeta}><span>{RESOURCE_KIND_LABELS[proposal.kind]}</span>{group.suggestion?.source === "model" ? <span className={styles.proposalSource}>保留 AI 建议</span> : group.proposal.adoptedFields.length > 0 && group.suggestion ? <span className={styles.proposalSource}>规则分类</span> : null}{showAI ? <span className={resultStyles.rowState}>{{ ready: "建议收藏", review: "需你确认", skipped: "已跳过", collected: "已收藏" }[group.resultStatus]}</span> : null}{proposal.tags.length ? <span>{proposal.tags.slice(0, 3).join(" · ")}</span> : null}</div>{group.suggestion?.source === "model" && !group.readOnly && group.proposal.retainedFields.length > 0 ? <p className={resultStyles.manualNote}>人工填写的内容已保留。</p> : null}{group.resultStatus === "review" || group.resultStatus === "skipped" ? <p className={styles.resultReason}>{group.resultReasons.join("；")}</p> : null}
  ...
  return <div className={styles.simplifiedWorkbench}>
    ...
    <section ref={resultsPanel} className={styles.resultsPanel} aria-label="书签整理结果">
      <nav className={styles.resultTabs} aria-label="整理结果视图">{VIEWS.map(item => <button key={item.id} type="button" aria-pressed={view === item.id && !showCollected && !showAI} onClick={() => changeView(item.id)}>{item.label}<span>{tabsCount(item.id).toLocaleString()}</span></button>)}</nav>
      {showAI ? <div className={resultStyles.aiView}><div><h2 ref={aiHeading} tabIndex={-1}>AI 建议{listData ? ` · 当前显示 ${listData.total} 个资源` : ""}</h2><p>查看本批目前保留 AI 建议的资源，包含不同整理状态。列表展示当前拟用内容，人工修改优先；查看不会扩大已选范围。</p></div><button type="button" className={styles.textLink} onClick={() => changeView("ready")}>返回建议收藏</button></div> : null}
      ...
      {showCollected ? <div className={styles.resultListToolbar}><strong>本批已收藏 {counts?.collected ?? 0} 个资源</strong><button type="button" className={styles.textLink} onClick={() => changeView("ready")}>返回待收藏结果</button></div> : !showAI && view === "ready" ? <div className={styles.resultListToolbar}><label className={styles.checkLabel}><input type="checkbox" checked={allReadySelected} disabled={disabled || !readyIds.length} onChange={toggleAllReady} />{allReadySelected ? `已选全部 ${readyIds.length} 个建议资源` : `选择全部 ${readyIds.length} 个建议资源`}<span>包含其他页</span></label><span className={styles.help}>分类可直接调整，无需逐条确认。</span></div> : !showAI && view === "review" ? <p className={styles.reviewIntro}>补充分类或处理重复关系后，资源会进入“建议收藏”，由你选择加入。</p> : null}
