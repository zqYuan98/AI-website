...
export function ManagerEditor({ resource, onClose, onSaved }: { resource: LibraryResource | null; onClose: () => void; onSaved: (resources: LibraryResource[], message: string) => void }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await managerRequest<{ resources: LibraryResource[] }>({ action: "save", resource: {
        ...(resource ?? {}),
      ...
      onSaved(result.resources, resource ? "修改已保存在本机。公开页需重新生成公开版本后更新。" : "已加入仅自己可见的待整理收藏。");
      onClose();
    } catch (failure) { setError(managerError(failure)); }
    finally { setBusy(false); }
  }

  return <ManagerDialog title={resource ? "整理这份收藏" : "添加一个资源"} description={resource ? "把用途写清楚，下次回来更容易找到。私人备注始终只留在本机。" : "先记下名称和网址，其他信息以后慢慢补。新资源仅自己可见。"} onClose={onClose} busy={busy}>
    <form onSubmit={save} className={styles.form}>
      ...
      <label className={styles.privateField}>私人备注 <span className={styles.smallBadge}>仅本机</span><textarea name="notes" rows={3} maxLength={10000} defaultValue={resource?.notes} placeholder="自己的想法、使用笔记、待办……" /></label>
      <div className={styles.formGrid}>
      ...
      <footer className={styles.dialogActions}><button type="button" className={styles.secondaryButton} onClick={onClose} disabled={busy}>取消</button><button type="submit" className={styles.primaryButton} disabled={busy}>{busy ? "正在保存…" : "保存到本机"}</button></footer>
    </form>
  </ManagerDialog>;
}
