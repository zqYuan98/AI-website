...
import { managerError, managerErrorStatus, managerRequest, type ManagerPublishResult, type ManagerRequestContext } from "./manager-api";
import { ManagerDialog, ManagerErrorNotice } from "./manager-primitives";
import styles from "./manager-workspace.module.css";

export function ManagerPublish({ resources, context, onClose, onPublished }: { resources: LibraryResource[]; context: ManagerRequestContext; onClose: () => void; onPublished: (result: ManagerPublishResult, message: string) => void }) {
  const [requestContext] = useState(() => ({ ...context }));
  const [preview, setPreview] = useState<(PublishPreview & { libraryRevision?: string }) | null>(null);
  ...
  useEffect(() => {
    ...
    managerRequest<PublishPreview & { libraryRevision?: string }>({ action: "publish-preview" }, requestContext, controller.signal)
      .then(setPreview).catch(failure => { if (!controller.signal.aborted) { setError(managerError(failure)); setErrorStatus(managerErrorStatus(failure)); } })
      .finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  }, [version, requestContext]);
  function refresh() { setError(""); setPreview(null); setLoading(true); setVersion(value => value + 1); }
  async function publish() {
    if (!preview || busy) return;
    setBusy(true); setError("");
    try {
      const result = await managerRequest<ManagerPublishResult>({ action: "publish", revision: preview.revision }, { ...requestContext, libraryRevision: preview.libraryRevision ?? requestContext.libraryRevision });
      const feedback = requestContext.mode === "cloud"
        ? result.cacheStatus === "pending" ? "公开版本已保存，网站缓存刷新尚未完成。可以重试刷新，无需再次发布。" : `已发布 ${result.count} 条资源。打开公开页查看最新版本。`
        : `已生成 ${result.count} 条资源的公开版本。现有部署流程完成后，线上网站才会更新。`;
      onPublished({ ...result, publishedIds: result.publishedIds ?? preview.resources.map(item => item.id) }, feedback);
      onClose();
  ...
  return <ManagerDialog title="检查这次公开的内容" description="只有下面预览中的公开字段会进入网站。私人备注、原书签文件夹和导入记录不会发布。" onClose={onClose} busy={busy} wide>
    <div className={styles.publishBody}>
      <p className={styles.publishNote}>{requestContext.mode === "cloud" ? "确认后会将本次预览发布到网站。平时保存的修改不会自动公开；发布无需重新部署代码。" : "生成的是本地公开版本。部署后线上生效；此操作不会执行部署。"}</p>
      ...
      {error ? <><ManagerErrorNotice message={error} status={errorStatus} /><button type="button" className={styles.secondaryButton} disabled={busy} onClick={refresh}>重新获取预览</button></> : null}
    </div>
    <footer className={styles.dialogActions}><button type="button" className={styles.secondaryButton} disabled={busy} onClick={onClose}>继续整理</button><button type="button" className={styles.primaryButton} disabled={busy || loading || !preview} onClick={publish}>{busy ? requestContext.mode === "cloud" ? "正在发布…" : "正在生成…" : requestContext.mode === "cloud" ? "确认发布到网站" : "生成公开版本"}</button></footer>
  </ManagerDialog>;
}
