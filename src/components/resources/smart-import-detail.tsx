...
export function SmartImportFieldsEditor({ context, groupIds, initial, onClose, onChanged }: { context: SmartImportBatchContext; groupIds: string[]; initial?: SmartImportFields; onClose: () => void; onChanged: (value: SmartImportBatchContext) => void }) {
  ...
  return <ManagerDialog title={initial ? "编辑资源内容" : `批量调整 ${groupIds.length} 个资源`} description="修改会保留在整理结果中，加入收藏时使用。" onClose={onClose} busy={busy}><form className={styles.dialogBody} onSubmit={save}><fieldset className={styles.formFields} disabled={busy}>
    ...
    <SmartFeedback error={error} status={status} /><div className={styles.dialogActions}><button type="button" className={styles.secondaryButton} onClick={onClose}>返回</button><button type="submit" className={styles.primaryButton} disabled={!Object.keys(fields).length}>{busy ? "正在保存…" : "保存修改"}</button></div>
...
export function SmartImportDetail({ context, groupId: initialGroupId, onClose, onChanged, onRegrouped }: { context: SmartImportBatchContext; groupId: string; onClose: () => void; onChanged: (value: SmartImportBatchContext) => void; onRegrouped: (value: SmartImportBatchContext) => void }) {
  ...
  const display = group?.proposal.fields;
  return <><ManagerDialog title={display?.name || "资源详情"} description="查看拟用内容、原始来源及重复关系。" onClose={onClose} busy={busy} wide><div className={styles.dialogBody}>
    <SmartFeedback error={error} status={status} />{loading ? <p className={styles.loading} role="status">正在读取候选…</p> : null}
    {group && data && display ? <><div className={styles.detailLead}><span className={styles.statusBadge}>{{ ready: "建议收藏", review: "需你确认", skipped: "已跳过", collected: "已收藏" }[group.resultStatus]}</span><p className={styles.fullUrl}>{group.representative.url}</p>{display.description ? <p>{display.description}</p> : null}<div className={styles.chips}><span>{RESOURCE_KIND_LABELS[display.kind]}</span><span>{display.category}</span>{display.tags.map(tag => <span key={tag}>{tag}</span>)}</div></div>
      {group.readOnly ? <p className={styles.notice}>{group.outcome && ["linked", "restored", "merged"].includes(group.outcome.kind) ? "这里显示原导入内容，处理后的收藏请打开查看。" : "本资源已有处理结果。后续修改可在收藏中进行。"}{group.outcome?.resourceId ? <Link href={`/tools/manage?resourceId=${encodeURIComponent(group.outcome.resourceId)}`} className={styles.textLink}>打开对应收藏 →</Link> : null}</p> : <><div className={styles.inlineActions}><button type="button" className={styles.secondaryButton} disabled={busy || loading} onClick={() => setEdit(true)}>编辑内容</button>{group.resultStatus === "review" && group.matchKind !== "archived" ? <button type="button" className={styles.secondaryButton} disabled={busy || loading} onClick={() => void mutate({ action: "decide", groupIds: [group.id], decision: "keep" })}>{group.suspectedMatches.length || group.suspectedGroupCount ? "已比较，保留为独立资源" : "确认当前内容"}</button> : null}<button type="button" className={styles.textLink} disabled={busy || loading} onClick={() => void mutate({ action: "decide", groupIds: [group.id], decision: group.decision === "ignore" ? "defer" : "ignore" })}>{group.decision === "ignore" ? "恢复待整理" : "忽略此资源"}</button></div>
      {group.resultReasons.length ? <p className={styles.notice}>{group.resultReasons.join("；")}</p> : null}
      {group.suggestion ? <details className={styles.compactDetails}><summary>{group.suggestion.source === "model" ? "AI 建议依据" : "分类依据"}</summary><p className={styles.help}>{group.suggestion.reason}</p><p className={styles.help}>上方已展示将使用的内容，人工修改优先。加入收藏时一并保存。</p></details> : null}</>}
