...
检查所有公开列表页及内容详情页（目前 13 个），要求状态 200、唯一 H1 和非空页面标题；本机管理页及两个管理 API 必须为 404。公开页面使用 GET；管理 API 检查 GET、POST、HEAD、OPTIONS、PUT、PATCH、DELETE，POST 只携带列表读取动作，不提交收藏写入或发布操作。可将参数替换成已部署站点的最终 origin（例如 `https://www.notvitamin.com`）执行相同检查；不自动跟随重定向，避免错误路径跳回首页后误报通过。
